...
test_capabilities_empty_force_color_enables_basic_color() {
    FORCE_COLOR=''
    unset NO_COLOR MODERN_BASH_COLOR
    TERM=dumb
    modern_bash::capabilities::detect 1 || return
...
test_capabilities_reject_closed_fd() {
    local status=0

    exec 98>&-
    modern_bash::capabilities::detect 98 || status=$?
...
test::run 'an empty FORCE_COLOR enables base ANSI colour' test_capabilities_empty_force_color_enables_basic_color
...
test::run 'closed file descriptors are rejected' test_capabilities_reject_closed_fd
